...
export function DataTableFilterMenu<TData>({
  ...
  // Detect client-side mode from table meta
  const enableClientSide = table.options.meta?.enableClientSide;

  // Client-side mode: use local state
  const [localFilters, setLocalFilters] = React.useState<
    ExtendedColumnFilter<TData>[]
  >([]);

  // Server-side mode: use URL state
  ...
  // Pick filter state based on mode
  const filters = enableClientSide ? localFilters : urlFilters;
  const setFiltersInternal = enableClientSide
    ? setLocalFilters
    : debouncedSetUrlFilters;

  // Sync local filters with table's columnFilters for client-side mode
  React.useEffect(() => {
    if (enableClientSide) {
  ...
  }, [enableClientSide, localFilters, table]);
  ...
  const onFilterAdd = React.useCallback(
    (column: Column<TData>, value: string) => {
      ...
      setFiltersInternal([...filters, newFilter]);
      setOpen(false);
